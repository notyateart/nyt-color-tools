<template>
  <section class="space-y-8">
    ...
    <!-- === Inputs === -->
    <form class="grid max-w-4xl gap-6 sm:grid-cols-2">
      <ColorInput
    ...
    <!-- === Preview Grid === -->
    <div
      class="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4 md:grid-cols-6 lg:grid-cols-8"
    >
      <div
        v-for="(color, i) in interpolated"
        :key="i"
        class="flex h-36 flex-col items-center justify-center gap-1 rounded border px-1 py-2 text-center font-mono text-xs shadow"
        :style="{ backgroundColor: color.hex, border: color.border }"
      >
        <span
          :style="{ color: color.text }"
          class="cursor-pointer hover:underline"
          @click="copy(color.oklch, 'OKLCH')"
        >
          {{ color.oklch }}
        </span>
        <span
    ...
    <div
      v-if="toast"
      class="fixed right-4 bottom-4 rounded bg-gray-900 px-4 py-2 text-sm text-white shadow transition-opacity duration-300"
    >
      {{ toast }}
    </div>
  </section>
